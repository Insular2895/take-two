...
def _candidate_markdown(candidate: CompiledStrategyCandidate) -> list[str]:
    ...
    lines = [
        ...
        f"- Budget remaining: `{candidate.risk.budget_remaining:.2f} "
        f"{'EUR' if candidate.risk.budget_remaining else 'request currency'}`",
    ...
    for metric in metrics:
        lines.append(
            f"- `{metric.model_id}`: P(gain) {metric.probability_profit:.1%}; "
            f"P(+50/+80/+100%) {metric.probability_gain_50:.1%}/"
            f"{metric.probability_gain_80:.1%}/{metric.probability_gain_100:.1%}; "
            f"P(-50/-70%) {metric.probability_loss_50:.1%}/"
            f"{metric.probability_loss_70:.1%}; expected {metric.expected_pnl:.2f}; "
